...
st.markdown("Enter current pollutant and time details to predict next AQI")
...
if "season" in df.columns:
    seasons = sorted(
        df["season"].dropna().astype(str).str.lower().unique().tolist()
    )
else:
    seasons = [
        "winter",
...
if st.button("Predict AQI 🌫️"):
    ...
    with st.spinner("Generating prediction..."):
        ...
        predicted_aqi = max(0, float(result[0]))
    ...
    st.success(f"Predicted Next AQI: **{predicted_aqi:.2f}**")

    if predicted_aqi <= 50:
        st.info("Air Quality Category: Good")

    elif predicted_aqi <= 100:
        st.success("Air Quality Category: Satisfactory")

    elif predicted_aqi <= 200:
        st.warning("Air Quality Category: Moderate")

    elif predicted_aqi <= 300:
        st.warning("Air Quality Category: Poor")

    elif predicted_aqi <= 400:
        st.error("Air Quality Category: Very Poor")

    else:
        st.error("Air Quality Category: Severe")
